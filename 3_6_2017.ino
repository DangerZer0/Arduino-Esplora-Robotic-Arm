...
Servo myservoX, myservoY, myservoZ, myservoH;  // Create 4 servo objects to control the servos connected to the Arduino Esplora
...
void setup()
{
  ...
  zDefServ = 180;              // Default servo Z angle
  ...
  yCurrJoy = map(yCurrServ, 180, 0, -512, 512) - xCalJoy;   // Set the current joystick Y reference position
  zCurrJoy = map(zCurrServ, 0, 180, -512, 512) - xCalJoy;   // Set the current joystick Z reference position 
...
void backToDef() {
  ...
  while ((xNewServ != xCurrServ) || (yNewServ != yCurrServ)){
   ...
   yCurrJoy = map(yCurrServ, 180, 0, -512, 512);
}
...
void loop(){
  ...
  myservoY.write(map(yCurrJoy, -512, 512, 180, 0));   // Write the Y position to the servo
